...
#Sélection du jeu de données pour avoir un jeu de données déséquilibrés
set.seed(123)
...
#Affichage de la plus haue valeur 
...
ggplot(results, aes(x = factor(nu), y = factor(gamma), fill = Kappa)) +
  geom_tile() +
  scale_fill_gradient2(high = "red", mid = "yellow", low = "darkblue", midpoint = 0.15) +
  labs(title = "Heatmap de kappa",
       x = "Valeurs de nu",
       y = "Valeurs de gamma",
       fill = "kappa") +
  theme_minimal()
...
for (fold in 1:10) {
  ...
  # Entraînez le modèle sur les données Diagnosis == 0 uniquement
  X_train <- train_fold_0[, -which(names(train_fold_0) == "Diagnosis")]
  model <- svm(X_train, type = "one-classification", nu=0.02, gamma=0.05)
