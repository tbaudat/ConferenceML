...
Prediction<- function(seuil, k){
  ...
  Local_density <- LOF(data = data_reduit[,3:32], k = k)
...
BalAcc <- function(k, seuil){
  ...
  Res <- confusionMatrix(table(pred, data_reduit$Diagnosis))
...
seuils <- seq(from = 1, to = 2.4, length = 15)
...
BalAcc(k=10, seuil = 1.2)

confusionMatrix(Prediction(seuil = 1.2, k= 10), data_reduit$Diagnosis)


#Resultats avec hyperprametres trouves ----
library(pROC)

data_reduit$pred <- as.numeric(Prediction(seuil = 1.2, k=10))-1
...
data_reduit[,34] <- LOF(data = data_reduit[,3:32], k = 10)
colnames(data_reduit)[34] <- "LOF"

ACP <- PCA(data_reduit[2:34], quali.sup = c(1, 32), quanti.sup = 33,
           graph = FALSE)
...
confusionMatrix(table(Prediction(seuil = 1.2, k= 10), data_reduit$Diagnosis))
